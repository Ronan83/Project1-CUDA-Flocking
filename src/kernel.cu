...
void Boids::initSimulation(int N) {
  ...
  checkCUDAErrorWithLine("cudaMemset dev_posSorted failed!");

  cudaMalloc((void**)&dev_velSorted, N * sizeof(glm::vec3));
  checkCUDAErrorWithLine("cudaMemset dev_velSorted failed!");
...
__device__ glm::vec3 computeVelocityChange(int N, int iSelf, const glm::vec3* pos, const glm::vec3* vel) {
    ...
    if (count1 > 0) {
        ...
        result += (perceivedCenter - thisPos) * rule1Scale;
    }
...
__global__ void kernUpdateVelNeighborSearchScattered(
  ...
  // - Clamp the speed change before putting the new speed in vel2
       ...
       for (int j = yMin; j <= yMax; j++) {
           for (int i = xMin; i <= xMax; i++) {
               ...
               for (int m = start; m <= end; m++) {
                   int b = particleArrayIndices[m];

                   if (b == index) continue;
                   float dist = glm::distance(pos[b], thisPos);
   ...
   if (count1 > 0) {
       perceivedCenter /= count1;
       velChange += (perceivedCenter - thisPos) * rule1Scale;
   }
...
__global__ void kernReshuffleData(int N, int* particleArrayIndices,
    ...
    posSorted[index] = pos[b];
    velSorted[index] = vel[b];
        ...
        for (int j = yMin; j <= yMax; j++) {
            for (int i = xMin; i <= xMax; i++) {
                ...
                for (int m = start; m <= end; m++) {
                    ...
                    if (m == index) continue;
                    float dist = glm::distance(pos[m], thisPos);

                    if (dist < rule1Distance) {
                        perceivedCenter += pos[m];
                        count1++;
                    }

                    if (dist < rule2Distance) {
                        separate -= (pos[m] - thisPos);
                    }

                    if (dist < rule3Distance) {
                        perceivedVelocity += vel1[m];
                        count3++;
                    }
                }
            }
...
void Boids::stepSimulationScatteredGrid(float dt) {
  ...
  // - Ping-pong buffers as needed
    ...
    // 5 kernUpdateVelNeighborSearchScattered
